...
# 페이지 설정
st.set_page_config(page_title="V-GEN VPP 수익 분석기 v5.7", layout="wide")
...
with st.sidebar:
    ...
    vgen_fee_rate = st.slider("상환 후 수수료율 (%)", 0, 50, 20)

    st.header("🛠️ 6. 참여 비용 (수익 상환)")
    st.success("✅ RTU/신자취(300만)는 VPP 추가 수익으로 우선 상환됩니다. 상환 완료 전까지 사업주 지출은 0원이며, 상환 완료 후 수수료 정산이 시작됩니다.")

# --- 3. 수익 계산 로직 ---
annual_gen = cap_mw * 1000 * gen_time * 365
# 상환 시뮬레이션을 위해 수수료 차감 전 순수 증분 계산
...
def generate_pro_report():
    ...
    reasons = [
        ...
        ("수익 기반 선상환 모델", "설치비 300만원은 VPP 참여로 발생하는 '새로운 수익'에서 먼저 차감됩니다. 즉, 기존 수익은 건드리지 않습니다."),
        ("상환 후 지속적 보너스", "구축비 상환이 완료되면, 발생되는 추가 수익은 약정된 수수료를 제외하고 모두 사업주의 순수익이 됩니다."),
        ("리스크 없는 참여", "초기 자본 투입이 없으므로 사업주는 손실 리스크 0% 상태에서 신규 수익원(CP/MAP/MWP)을 확보하게 됩니다.")
    ]
...
# --- 5. 메인 UI ---
st.title("📑 V-GEN VPP 수익 분석 대시보드 v5.7")

m1, m2, m3 = st.columns(3)
m1.metric("기존 연간 수익", f"{(annual_gen * fixed_p)/10000:,.0f} 만원")
m2.metric("VPP 상환 후 예상 수익", f"{total_rev_vpp/10000:,.0f} 만원", f"+{net_increase/10000:,.0f} 만원")
m3.metric("초기 투자 지출", "0원", f"수익상환기간: 약 {payback_months:.1f}개월")

st.divider()

c1, c2 = st.columns([1.5, 1])
with c1:
    st.subheader("📊 정산 항목별 수익 구성 (상환 완료 후 기준)")
...
with c2:
    st.subheader("📋 실시간 상환 시뮬레이션")
    st.info(f"📍 **예상 상환 소요 기간:** 약 **{payback_months:.1f}개월**")
    st.caption("※ VPP 참여로 발생하는 '추가 정산금' 전액을 설치비 상환에 우선 활용하는 기준입니다.")
    with st.expander("💰 상환 상세 안내"):
        st.write(f"- 초기 비용 합계: 300만원")
        st.write(f"- 월 평균 추가 수익: {annual_gross_extra/120000:,.1f} 만원")
        st.write(f"- **상환 완료 전**: 사업주 지출 0원, 기존 매전 수익 100% 유지")
        st.write(f"- **상환 완료 후**: 수수료 {vgen_fee_rate}% 제외 후 추가 수익 수령")
...
st.table(pd.DataFrame({
    ...
    "기본 매전": [f"{annual_gen:,.0f} kWh", f"{fixed_p:,.1f} 원", f"{(annual_gen * fixed_p)/10000:,.0f} 만원", "-"],
    "브이젠 VPP": [f"{annual_gen:,.0f} kWh", f"{(fixed_p + owner_net_extra_unit):,.2f} 원", f"{total_rev_vpp/10000:,.0f} 만원", "0원 (선수익 상환 방식)"]
}))
